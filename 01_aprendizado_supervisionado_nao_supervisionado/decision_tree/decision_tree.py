...
Este arquivo tem duas partes:
  1) Uma aula "mastigada", com analogias (Chapéu Seletor, Akinator,
     potinho de bala) e um exemplo de brincadeira com Pokémon, pra
     entender a ideia ANTES de ver a matemática rodando em cima de dados
     de verdade. Não exige conhecimento prévio de programação ou estatística.
...
PENSE NO CHAPÉU SELETOR DE HOGWARTS.

Ele não escaneia sua alma inteira de uma vez e já cospe "Grifinória!". Ele
vai sondando aos poucos: "você valoriza mais coragem ou esperteza?",
"prefere agir ou planejar?". Cada resposta elimina possibilidades, até
sobrar só UMA casa. Uma árvore de decisão é exatamente isso, só que
aplicada a dados: uma sequência de perguntas organizadas de um jeito que
chega numa resposta o mais rápido possível.

No vocabulário "oficial":

    - Nó interno  -> uma PERGUNTA sobre um atributo (ex.: "É do Tipo Água?")
    - Ramo        -> uma das respostas possíveis daquela pergunta
    - Nó folha    -> a DECISÃO FINAL (a classe prevista)

Pra classificar um caso novo, começa-se no topo (raiz) e vai-se
respondendo pergunta por pergunta até cair numa folha. Esse caminho é
literalmente um fluxograma: "sim -> desce por aqui, não -> desce por ali".

1) COMO A ÁRVORE É CONSTRUÍDA (indução top-down)
   O "Chapéu Seletor" da árvore não nasce sabendo quais perguntas fazer.
   Ele aprende, olhando pros exemplos de treino, qual é a melhor pergunta
   pra começar, depois a melhor pra cada ramificação, e assim por diante:

       a. Olha pro grupo de exemplos que chegou até aqui. Todos são da
          mesma classe? -> Vira uma FOLHA (decisão pronta, não precisa
          perguntar mais nada).
       b. Se não, testa TODAS as perguntas possíveis e escolhe a que mais
          separa o grupo em subgrupos "mais parecidos entre si" (menos
          misturados).
       c. Divide o grupo pelas respostas dessa pergunta e repete o
          processo (a, b, c...) dentro de cada subgrupo, do zero.

   Isso é feito de cima pra baixo (raiz -> folhas) e é GULOSO: a cada
   pergunta, o algoritmo escolhe a melhor opção NAQUELE MOMENTO e nunca
   volta atrás pra reconsiderar. Como alguém jogando um jogo de
   perguntas e respostas sem poder desfazer uma pergunta já feita.

2) COMO A ÁRVORE SABE QUAL É A "MELHOR" PERGUNTA?

   Ideia central, numa frase: a árvore prefere a pergunta que deixa os
   subgrupos resultantes MAIS "ARRUMADOS" (menos misturados).

   Imagine dois potinhos de bala:
       Potinho A: 20 balas, todas de morango.       -> bem "arrumado"
       Potinho B: 20 balas, 10 sabores diferentes.   -> uma bagunça

   "Impureza" é justamente essa medida de bagunça de um grupo. Um grupo
   com uma classe só (só bala de morango) tem impureza ZERO. Um grupo
   50%/50% de duas classes é o mais "bagunçado" possível: máxima
   impureza. Entropia e Gini, abaixo, são só duas réguas diferentes pra
   medir essa bagunça, e no fim quase sempre concordam sobre qual
   pergunta é a melhor.

   a. ENTROPIA: "o nível de dúvida do Chapéu Seletor"

      Fórmula "oficial"

          Entropia(S) = - Σ p_i * log2(p_i)

      onde p_i é a fração de exemplos da classe i dentro do grupo S.

      Pensa assim: já jogou Akinator (aquele gênio de site que
      adivinha em quem você está pensando fazendo perguntas de sim/não)?
      Entropia é basicamente "quantas perguntas de sim/não o Akinator
      precisaria, em média, pra descobrir a classe certa dentro desse
      grupo, jogando do jeito mais esperto possível".

          - Grupo com 1 classe só (ex.: só bala de morango) -> Entropia
            = 0. Zero perguntas: eu já sei a resposta de cara.
          - Grupo 50/50 (2 classes, bem misturado) -> Entropia = 1.
            Preciso de exatamente 1 pergunta boa ("é da classe A?") pra
            resolver: a MÁXIMA incerteza possível com 2 classes.
          - Grupo desbalanceado (ex.: 90% morango, 10% uva) -> Entropia
            fica baixinha: se eu simplesmente "chutar morango" sempre,
            acerto quase sempre, então já tenho pouca dúvida a resolver.

      De onde vem essa conta? Cada termo -log2(p_i) mede a "SURPRESA" de
      ver a classe i: quanto mais RARO um evento, mais surpreendente ele
      é quando acontece (ninguém se surpreende com "o sol nasceu hoje";
      todo mundo se surpreende com "nevou no Rio de Janeiro"). Em bits:

          -log2(1)      = 0 bit   -> evento certo, zero surpresa
          -log2(1/2)    = 1 bit
          -log2(1/4)    = 2 bits
          -log2(1/8)    = 3 bits

      Cada vez que a chance do evento cai pela metade, a surpresa sobe 1
      bit. A ENTROPIA é a MÉDIA dessa surpresa dentro do grupo, ponderada
      pela própria chance de cada classe aparecer.

   b. GANHO DE INFORMAÇÃO (usado no ID3 e no C4.5)

      É simplesmente: "quanto a dúvida (entropia) DIMINUIU depois que eu
      fiz essa pergunta?"

          Ganho(S, pergunta) = Entropia(S) - Entropia MÉDIA dos subgrupos

      A árvore testa essa conta pra CADA pergunta candidata e escolhe a
      que derruba mais a dúvida, a que mais "arruma a bagunça".
      (Detalhe de prova: o Ganho de Informação tende a favorecer
      perguntas com muitas respostas possíveis; o C4.5 corrige isso
      dividindo pelo "Split Info", virando a Razão de Ganho / Gain Ratio.)

   c. ÍNDICE GINI (usado no CART; é o padrão do scikit-learn)

          Gini(S) = 1 - Σ p_i²

      Outra régua de "bagunça", com leitura diferente: é a chance de eu
      errar a classe se eu pegasse uma bala aleatória do potinho e
      "apostasse" um rótulo sorteado seguindo a MESMA mistura do potinho.
      Potinho de uma cor só -> Gini = 0 (impossível errar). Potinho bem
      misturado -> Gini alto (fácil errar no "chute"). A árvore escolhe a
      pergunta que mais DIMINUI o Gini médio dos subgrupos. É mais rápido
      de calcular que a entropia (não usa logaritmo) e, na prática, quase
      sempre concorda com ela.

      Pegadinha clássica de prova: as duas réguas valem 0 num grupo puro
      e são MÁXIMAS no caso 50/50 (2 classes), mas o valor máximo é
      DIFERENTE: Entropia chega a 1, Gini chega só a 0,5. Isso raramente
      muda qual pergunta é escolhida, só muda a escala do número.

3) OS TRÊS "SOTAQUES" DO ALGORITMO
   - ID3  (Quinlan, 1986): só atributos categóricos, usa Ganho de
     Informação, não poda, não sabe lidar com dado faltando.
   - C4.5 (Quinlan, 1993): evolução do ID3; usa Razão de Ganho, lida com
     números contínuos (cortando em faixas) e com dado faltando, e faz
     PÓS-PODA.
   - CART (Breiman et al., 1984): só faz perguntas de SIM/NÃO (árvore
     sempre binária); usa Gini pra classificação (ou erro quadrático pra
     prever números). É o que o scikit-learn implementa.

4) "DECOREBA" DEMAIS VIRA PROBLEMA: OVERFITTING E PODA
   Se a árvore puder crescer sem limite, ela vai fazendo perguntas cada
   vez mais específicas até "decorar" cada exemplo de treino de cor,
   inclusive os erros e ruídos dos dados. Na hora de prever um caso NOVO,
   ela vai mal, porque aprendeu detalhe demais em vez de padrão de
   verdade. Isso é overfitting. Duas formas de evitar:

   - PRÉ-PODA: freia o crescimento enquanto a árvore ainda está sendo
     construída: limita a profundidade máxima (max_depth), exige um
     número mínimo de exemplos por folha (min_samples_leaf), etc.
   - PÓS-PODA: deixa a árvore crescer inteira e depois "apara os galhos"
     que não ajudam de verdade a performance (ex.: Cost-Complexity
     Pruning, no scikit-learn via o parâmetro `ccp_alpha`).

5) VANTAGENS x DESVANTAGENS
   (+) Fácil de entender e desenhar (é literalmente um fluxograma).
   (+) Não precisa normalizar/escalonar os dados antes.
   (+) Lida bem com número e categoria ao mesmo tempo.
   (-) Instável: mudar um pouco os dados de treino pode gerar uma árvore
       bem diferente.
   (-) Tende a "decorar" demais (overfitting) se não for podada.
   (-) É gulosa -> não garante ser a MELHOR árvore possível, só a melhor
       escolha local a cada passo.
   (-) Tende a "chutar sempre a classe mais comum" quando os dados são
       desbalanceados (relevante aqui: fraude é raríssima!).
   Observação: Random Forest e Gradient Boosting juntam várias árvores
   pra compensar essa instabilidade. É o próximo capítulo da história.
---------------------------------------------------------------------------
"""
