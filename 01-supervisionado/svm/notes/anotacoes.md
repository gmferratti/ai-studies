...
Imagine uma folha de papel com bolinhas desenhadas formando um alvo de dardo: um monte de bolinhas azuis formando um círculo bem no meio, e bolinhas vermelhas formando um anel em volta. Não tem como traçar uma linha reta nessa folha que separe todas as azuis das vermelhas, porque as azuis estão cercadas pelas vermelhas dos quatro lados. Agora imagine que você pega essa folha e empurra pra cima bem o centro dela, virando a folha plana numa espécie de morro: as bolinhas azuis do meio sobem junto, formando o topo, e as vermelhas do anel ficam lá embaixo, na base. Com a folha virada nesse morro em 3D, fica fácil separar: basta passar uma placa reta e plana na altura certa, cortando o morro no meio da subida, que ela separa perfeitamente as azuis (em cima) das vermelhas (embaixo). Olhando essa placa de cima, achatando tudo de novo em 2D, ela aparece como um círculo, uma linha curva, mesmo tendo sido um corte completamente reto lá em 3D.

É essa a ideia do kernel, só que em vez de "empurrar uma folha", existe uma função `φ` (fi) que "levanta" matematicamente cada ponto pra um espaço com mais dimensões, onde as classes que pareciam embaralhadas viram separáveis por um hiperplano reto (mesmo que esse hiperplano, quando projetado de volta pro espaço original, pareça uma curva). O problema é que calcular `φ(x)` explicitamente pode ser caro ou até ter dimensão infinita, o equivalente a "montar o morro de verdade" seria trabalhoso demais. O truque do kernel evita esse cálculo: em vez de calcular `φ(x)` pra cada ponto e depois multiplicar, existe uma função `K(x, x') = φ(x) · φ(x')` que calcula diretamente o produto final, sem nunca precisar montar o vetor `φ(x)` inteiro, tipo descobrir a distância entre dois pontos no morro usando só uma continha em cima das posições originais na folha, sem nunca precisar erguer o morro de verdade. Como o algoritmo do SVM (na forma dual, ver seção abaixo) só usa produtos entre pares de exemplos, basta trocar cada produto `x · x'` por `K(x, x')` em todas as contas, e o hiperplano linear no espaço "levantado" vira, de volta no espaço original, uma fronteira curva.
